...
def blackbox_old(circuit_index, circuits_list, network):
    """
...
def blackbox_new(circuit_index, circuits_list, network):
    """
    Computes the latency of a set of circuits running on a network.
    Uses caching to avoid redundant computations.
    """
...
if __name__ == "__main__":
    method_list = ['optimal_DP', 'min_k_DP', 'order_DP', 'merge_batches', 'SetCover1', 'ApproximateGreedy']
    random.seed(102)
    np.random.seed(102)
    network = NetworkGraph(M,num) # a class for network
    _distance_matrix=nx.floyd_warshall_numpy(network.network_graph)
    
    ################## change the input here
    used_method_index = [1,2,3,4,5]  
    used_circ_index = [13,12,11]
    ##################
