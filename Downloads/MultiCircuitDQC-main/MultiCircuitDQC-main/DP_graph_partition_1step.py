...
import cProfile
import pstats
from cluster import get_group_matrices_computer
...
def blackbox(circuit_index, circuits_list, network):
    ...
    n = len(index_key)
    group_matrix = _group_matrices[n]
    latency_list = []
    for i in range(n):
        ...
        if key in blackbox_cache:
            ...
            latency = MinQAP_group(circuits_list[index_key[i]], group_matrix[i])
...
def main():
    method_list = ['min_k_DP', 'merge_batches', 'SetCover1']
    random.seed(102)
    np.random.seed(102)
    network = NetworkGraph(M,num) # a class for network
    global _group_matrices
    __, _group_matrices = get_group_matrices_computer(network)
    ...
    for folder_num in used_circ_index:  
        ...
        for i in used_method_index:
            ...
            if method == 'SetCover1':
                best_batch3, best_time = SetCover1(circuit_list, network)
