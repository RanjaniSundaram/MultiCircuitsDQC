...
import cProfile
import pstats
from cluster import get_group_matrices
...
def blackbox(circuit_index, circuits_list, network):
    ...
    latency_list = []
    for i in range(n):
        latency = MinQAP_group(circuits_list[i], group_matrix[i])
        latency_list.append(latency)
    final_latency = max(latency_list)

    # Store the result in the cache
    blackbox_cache[key] = final_latency
...
def main():
    ...
    global _group_matrices
    _group_matrices = get_group_matrices(network)
